...
USR="switchon"    
PWD=""
SHP="ECRINS_vs2_Lakes"
LYR="ECRINS:ECRINS_lakes"
RES="307"    
...
ogr2ogr -progress -simplify 500 --config PG_USE_COPY YES -f PostgreSQL PG:"host=127.0.0.1 port=5432 dbname=$DB password=$PWD user=$USR" -lco DIM=2 $SHP.shp -sql "SELECT PK_UID FROM $SHP" -overwrite -lco OVERWRITE=YES -t_srs "EPSG:4326" -a_srs "EPSG:4326" -lco SCHEMA=import_tables -lco GEOMETRY_NAME=geom -nln geosearch_import -gt 65536 -nlt PROMOTE_TO_MULTI
