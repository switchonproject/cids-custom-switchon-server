...
CREATE INDEX tag_name_ftidx ON public.tag USING gin (to_tsvector('simple',  name));
CREATE INDEX taggroup_name_ftidx ON public.taggroup USING gin (to_tsvector('simple',  name));
