-- import the X-CUAHSI Tabluar Ontology into new X-CUAHSI keyword list.
-- skip duplicate keywords
INSERT INTO taggroup
(name, description)
SELECT 'keywords - X-CUAHSI', 'X-CUAHSI Hydrologic Ontology for Discovery'
WHERE
    NOT EXISTS (
        SELECT name FROM taggroup WHERE name = 'keywords - X-CUAHSI'
    );
DO $$
...
BEGIN
    taggroupID = (SELECT id FROM taggroup WHERE name = 'keywords - X-CUAHSI');
